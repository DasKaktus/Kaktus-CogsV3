...
_ = Translator("CustomCommandsImproved", __file__)
...
class CommandObjImproved:
    ...
    async def get(self, message: discord.Message, command: str.lower) -> Tuple[str.lower, Dict]:
        if not command:
            raise NotFound()
...
@cog_i18n(_)
class CustomCommandsImproved(commands.Cog):
    ...
    async def cc_search(self, ctx: commands.Context, *, query):
        ...
        cc_commands = await CommandObjImproved.get_commands(self.config.guild(ctx.guild))
    ...
    async def cc_create_random(self, ctx: commands.Context, command: str.lower):
        ...
        if command in (*self.bot.all_commands, *commands.RESERVED_COMMAND_NAMES):
            ...
            return
        responses = await self.commandobjimproved.get_responses(ctx=ctx)
        if not responses:
            await ctx.send(_("Custom command process cancelled."))
            return
        try:
            await self.commandobjimproved.create(ctx=ctx, command=command, response=responses)
    ...
    async def cc_create_simple(self, ctx, command: str.lower, *, text: str):
        ...
        if command in (*self.bot.all_commands, *commands.RESERVED_COMMAND_NAMES):
            ...
            return
        try:
            await self.commandobjimproved.create(ctx=ctx, command=command, response=text)
            await ctx.send(_("Custom command successfully added."))
        except AlreadyExists:
            await ctx.send(
                _("This command already exists. Use `{command}` to edit it.").format(
    ...
    async def cc_cooldown(
        ...
        if cooldown is None:
            try:
                cooldowns = (await self.commandobjimproved.get(ctx.message, command))[1]
            except NotFound:
        ...
        try:
            await self.commandobjimproved.edit(ctx=ctx, command=command, cooldowns=cooldown, ask_for=False)
    ...
    async def cc_delete(self, ctx, command: str.lower):
        ...
        """
        try:
            await self.commandobjimproved.delete(ctx=ctx, command=command)
    ...
    async def cc_edit(self, ctx, command: str.lower, *, text: str = None):
        ...
        """
        try:
            await self.commandobjimproved.edit(ctx=ctx, command=command, response=text)
    ...
    async def cc_list(self, ctx: commands.Context):
        ...
        """
        cc_dict = await CommandObjImproved.get_commands(self.config.guild(ctx.guild))
    ...
    async def cc_show(self, ctx, command_name: str):
        ...
        try:
            cmd = await self.commandobjimproved.get_full(ctx.message, command_name)
        except NotFound:
    ...
    async def get_command_names(self, guild: discord.Guild) -> Set[str]:
        ...
        """
        return set(await CommandObjImproved.get_commands(self.config.guild(guild)))
